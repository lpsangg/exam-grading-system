...
export default function Step1Page() {
  ...
  const { setAnswerFile, answerFile, setExamCodes, setAnswerKeys } = useExamStore()
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Lưu answer keys vào store
        if (data.answerKeys && typeof data.answerKeys === 'object') {
          setAnswerKeys(data.answerKeys)
          console.log('Saved answer keys to store:', data.answerKeys)
        }
        setUploadResult({
          success: true,
